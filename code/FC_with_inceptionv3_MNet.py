...
# Freez the first 62 layers and retrain the model again (62 means 20% of layers 310)
...
for filename in filenames:
  if("Good" in filename):
    categoryGoodBad.append("Good")
  if("Bad" in filename):
    categoryGoodBad.append("Bad")
...
# Freez the first 62 layers and retrain the model again (62 means 20% of layers 310)
for layer in modelGB.layers[:62]:
   layer.trainable = False
...
sgd = optimizers.SGD(lr=0.0001, momentum=0.9)
modelGB.compile(optimizer=sgd, loss='categorical_crossentropy', metrics=['accuracy'])

modelGB.summary()

history = modelGB.fit(train_generator,
                              validation_data=test_generator,
                              steps_per_epoch=total_train/batch_size,
                              epochs=5,
                              validation_steps=total_validate/batch_size,
...
model.save("results/FCInceptionV3_MNet_Fruit.h5")
modelGB.save("results/FCInceptionV3_MNet_Quality.h5")
